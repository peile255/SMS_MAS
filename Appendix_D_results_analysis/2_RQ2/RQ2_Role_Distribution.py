...
df = df.dropna(subset=['Agent_Role'])
...
df['Agent_Role'] = df['Agent_Role'].apply(try_parse)

# 5. 拆分多标签为一维列表，统计频次
roles_flat = [role.strip() for sublist in df['Agent_Role'] for role in sublist if str(role).strip().lower() not in ['nan', 'none', '']]
...
role_df.to_csv('figures/RQ2_Role_Distribution.csv', index=False)
print("数据已保存至：figures/RQ2_Role_Distribution.csv")
...
ax.set_ylabel('Role Type', labelpad=8, fontsize=12)
ax.set_title('Agent Role Distribution', pad=10, fontsize=14, weight='bold')
...
# 保存高质量图像
plt.savefig(
    'figures/RQ2_Role_Distribution.png',
    dpi=600,
    bbox_inches='tight',
    transparent=False
)
...
plt.show()
print("图表已保存至：figures/RQ2_Role_Distribution.png")
