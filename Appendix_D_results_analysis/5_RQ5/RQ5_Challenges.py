...
df['Challenges'] = df['Challenges'].apply(try_parse)

# 5. 拆分多标签为一维列表，统计频次
roles_flat = [role.strip() for sublist in df['Challenges'] for role in sublist if str(role).strip().lower() not in ['nan', 'none', '']]
...
role_df.columns = ['Challenges', 'Count']
role_df.to_csv('figures/RQ5_Challenges.csv', index=False)
print("数据已保存至：figures/RQ5_Challenges.csv")
...
ax.set_ylabel('Challenges', labelpad=8, fontsize=12)
ax.set_title('Challenges Distribution', pad=10, fontsize=14, weight='bold')
...
plt.show()
print("图表已保存至：figures/RQ5_Challenges.png")
