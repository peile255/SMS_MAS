...
df = df.dropna(subset=['Evaluation_Method'])
...
df['Evaluation_Method'] = df['Evaluation_Method'].apply(try_parse)

# 5. 拆分多标签为一维列表，统计频次
roles_flat = [role.strip() for sublist in df['Evaluation_Method'] for role in sublist if str(role).strip().lower() not in ['nan', 'none', '']]
...
role_df.columns = ['Evaluation_Method', 'Count']
role_df.to_csv('figures/RQ4_Evaluation_Method.csv', index=False)
print("数据已保存至：figures/RQ4_Evaluation_Method.csv")
...
ax.set_title('Evaluation Method Distribution', pad=10, fontsize=14, weight='bold')
...
# 保存高质量图像
plt.savefig(
    'figures/RQ4_Evaluation_Method.png',
    dpi=600,
    bbox_inches='tight',
    transparent=False
)
...
plt.show()
print("图表已保存至：figures/RQ4_Evaluation_Method.png")
